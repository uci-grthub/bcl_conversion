...
open(IN2,"$fastqR2"); while(my $l=<IN1>){ if($l!~/^@/){ print "1 not in FastQ file format.\n";
exit; } if($l=<IN2>){ if($l!~/^@/){ print "Input 2 not in FastQ file format.\n"; exit; } }
else{ print "Inputs are not paired-reads.\n"; exit; } $l=<IN1>; chomp($l); my @nts1=split("",$l);
my $len1=$#nts1+1; if($num_POS_R1==0){ $num_POS_R1=$len1; } elsif($len1!=$num_POS_R1){
print "Sequences must have same length.\n"; exit; } for(my $i=0;$i<$len1;$i++){ my $nt=$nts1[$i];
...
print OUT "set ylabel \"$phred_ylabel\" $phred_offset,0 $ylabel_font\n";
...
print OUT "set ytics nomirror\nset grid ytics\n";
print OUT "set ylabel \"$calls_ylabel\" $calls_offset,0 $ylabel_font\n";
