...
if($#ARGV!=3){ print "\n  USAGE : ./postprocess_hiseq_lane.pl ";
  print "sample_sheet num_reads lane_number library_name\n\n"; exit 1; }
...
my $process_single="/home/sequser/Facility/Tools/test/new_analyze_single_reads_centos7.pl";
my $process_paired="/home/sequser/Facility/Tools/analyze_paired_reads_centos7.pl";
...
# Process Lane (Case Multiplexed)
  ...
  for(my $i=0;$i<$num_barcodes;$i++){
    my $barcode=$barcodes[$i]; my $prefix=$prefixes[$i]; my $genid=$generic[$i];
    print "  Processing Library \"$name\" - Barcode $barcode ($prefix)...\n";
    my $file_in_R1="${prefix}_S${genid}_L00${lane}_R1_001.fastq";
    my $file_in_R2="${prefix}_S${genid}_L00${lane}_R2_001.fastq";
    my $file_in_R3="${prefix}_S${genid}_L00${lane}_R3_001.fastq";
    if(! -f "$file_in_R3"){ print "Cannot find fastq file\n"; exit 1; }
    if(($num_reads==2)&&(! -f "$file_in_R2")){ print "Cannot find fastq file\n"; exit 1; }
    
    ################################################################################################
    
    if($num_reads==1){
      my $file_out="${prefix}-${barcode}-READ3-Sequences.txt";
      `mv $file_in_R3 $file_out`;
      `$process_single $file_out ${prefix}-${barcode} \"$name\"`;
      `chmod 770 $prefix*`; `gzip $file_out; chmod 770 $file_out.gz`;
      `md5sum $file_out.gz >> md5sum_lane$lane.txt`; }
    
    ################################################################################################
    
    elsif($num_reads==2){
      my $file_out_R1="${prefix}-${barcode}-READ1-Sequences.txt";
      my $file_out_R2="${prefix}-${barcode}-READ2-Sequences.txt";
      `mv $file_in_R1 $file_out_R1`;
      `mv $file_in_R2 $file_out_R2`;
      `$process_paired $file_out_R1 $file_out_R2 ${prefix}-${barcode} \"$name\"`;
      `chmod 770 $prefix*`; `gzip $file_out_R1; chmod 770 $file_out_R1.gz`;
      `gzip $file_out_R2; chmod 770 $file_out_R2.gz`;
      `md5sum $file_out_R1.gz >> md5sum_lane$lane.txt`;
      `md5sum $file_out_R2.gz >> md5sum_lane$lane.txt`; } }
  
  ##################################################################################################
  
  my @d=split("-",$prefixes[0]); my $prefix="$d[0]-$d[1]";
  if($prefix!~/^(4R|mR|R)[0-9]{3}-L$lane$/){ print "Cannot process trash\n"; exit 1; }
  $prefix="$prefix-PrNotRecog";
  print "  Processing Library \"$name\" - Barcode Not Recognized ($prefix)...\n";
  my $file_in_R1="Undetermined_S0_L00${lane}_R1_001.fastq";
  my $file_in_R2="Undetermined_S0_L00${lane}_R2_001.fastq";
  my $file_in_R3="Undetermined_S0_L00${lane}_R3_001.fastq";
  ...
  if($num_reads==1){
    my $file_out="${prefix}-READ3-Sequences.txt";
    `mv $file_in_R3 $file_out`;
  ...
  ##################################################################################################
    ...
    my $file_out_R1="${prefix}-READ1-Sequences.txt";
    my $file_out_R2="${prefix}-READ2-Sequences.txt";
    `mv $file_in_R1 $file_out_R1`;
    `mv $file_in_R2 $file_out_R2`;
