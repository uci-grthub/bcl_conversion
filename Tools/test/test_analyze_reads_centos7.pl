...
# by Yuzo Kanomata                                                                       #
#                                                                                        #
# Test triple reads processing
##########################################################################################

# Script Behaviour
my $verbose=1;                 # Display messages or not
my $write_raw_stats_pos=0;     # Report raw statistics per position

##########################################################################################

# Script Inputs
my $script_usage="./analyze_reads.pl  fastqR1  fastqR2  fastqR3 prefix  [sample]";
if(($#ARGV!=2)&&($#ARGV!=4)){ print "\nUsage: $script_usage\n\n"; exit; }
my $fastqR1=$ARGV[0]; my $fastqR2=$ARGV[1]; my $fastqR2=$ARGV[2]; my $prefix=$ARGV[3]; my $library="$prefix";
...
my $gnuplot_callsR3_out="$prefix-READ3-BaseComposition.png";
...
my $num_POS_R2=0; my @POS_ID_R3; my @POS_PHRED_R3; my %POS_CALLS_R3;
...
else{ print "Inputs are not triple-reads.\n"; exit; } 

$l=<IN1>; chomp($l); my @nts1=split("",$l);
my $len1=$#nts1+1; if($num_POS_R1==0){ $num_POS_R1=$len1; } elsif($len1!=$num_POS_R1){
...
exit; } $POS_CALLS_R2{"$i-$nt"}++; } $l=<IN1>; $l=<IN1>; chomp($l); my @phred1=split("",$l);


if(($#phred1+1)!=$len1){ print "Scores length mismatch 1.\n"; exit; } 

for(my $i=0;$i<$len1;$i++){
$POS_PHRED_R1[$i]+=(int(ord($phred1[$i]))-$offset); } $l=<IN2>; $l=<IN2>; chomp($l);

my @phred2=split("",$l); if(($#phred2+1)!=$len2){ print "Scores length mismatch 2.\n"; exit; }

for(my $i=0;$i<$len2;$i++){ $POS_PHRED_R2[$i]+=(int(ord($phred2[$i]))-$offset); } $num_reads++; }
