...
my $process_single="/home/sequser/Facility/Tools/analyze_single_reads_centos7.pl";
my $process_paired="/home/sequser/Facility/Tools/analyze_paired_reads_centos7_test.pl";
...
# Process Lane (Case Multiplexed)
  ...
  for(my $i=0;$i<$num_barcodes;$i++){
    ...
    if($num_reads==1){
      my $file_out="${prefix}-${barcode}-Sequences.txt";
    ...
    print "  Processing check Library \"$name\" - Barcode $prefix -$barcode ...\n";
  ...
  if($num_reads==1){
    my $file_out="${prefix}-Sequences.txt";
    `mv $file_in_R1 $file_out`;
