...
def main():
    if len(sys.argv) != 5:
        print("\n  USAGE : ./postprocess_hiseq_lane.py sample_sheet num_reads lane_number library_name\n")
        sys.exit(1)
    ...
    process_single = os.path.join(tools_dir, "analyze", "analyze_single_reads.py")
    process_paired = os.path.join(tools_dir, "analyze", "analyze_paired_reads.py")
            ...
            for l in lines[2:]:
                ...
                if int(d[1]) == lane:
                    if d[4] == "":
                        ...
                        # I'll skip strict regex for now or implement if critical
    ...
    if is_barcoded == 0:
        ...
        for i in range(num_barcodes):
            ...
            if num_reads == 1:
                ...
                # chmod 770 prefix*
                # I'll skip chmod or do it if needed. The Perl script does it.
                subprocess.run(f"chmod 770 {prefix}*", shell=True)
                
                subprocess.run(["gzip", file_out], check=True)
                subprocess.run(f"chmod 770 {file_out}.gz", shell=True)
                
                with open(f"md5sum_lane{lane}.txt", "a") as md5:
                    subprocess.run(["md5sum", f"{file_out}.gz"], stdout=md5, check=True)
                ...
                subprocess.run(["gzip", file_out_R1], check=True)
                subprocess.run(f"chmod 770 {file_out_R1}.gz", shell=True)
                
                subprocess.run(["gzip", file_out_R2], check=True)
                subprocess.run(f"chmod 770 {file_out_R2}.gz", shell=True)
                
                with open(f"md5sum_lane{lane}.txt", "a") as md5:
                    subprocess.run(["md5sum", f"{file_out_R1}.gz"], stdout=md5, check=True)
                    subprocess.run(["md5sum", f"{file_out_R2}.gz"], stdout=md5, check=True)
