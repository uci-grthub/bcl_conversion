...
my $len=length($cand); if(($len>=4)&&($len<=10)&&($cand=~/^[ACGT]+$/)){
...
while(my $l=<IN>){ if($l!~/^@/){ print "Input not in FastQ file format.\n"; exit; }
$l=<IN>; chomp($l); my @nts=split("",$l); my $len=$#nts+1; $POS_NUMSQ[$len-1]++;
for(my $i=0;$i<$len;$i++){ my $nt=$nts[$i]; if($VALID{"$nt"}!=1){ print "Invalid base $nt.\n";
exit; } $POS_CUMUL[$i]++; $POS_CALLS{"$i-$nt"}++; } $avg_len+=$len; $l=<IN>; $l=<IN>;
chomp($l); my @phred=split("",$l); if(($#phred+1)!=$len){ print "Scores length mismatch.\n";
exit; } for(my $i=0;$i<$len;$i++){ $POS_PHRED[$i]+=(int(ord($phred[$i]))-$offset); }
...
if($verbose){ print "  Finalizing stats per position...\n\n"; } for(my $i=0;$i<=$#POS_NUMSQ;$i++){
my $pos=$i+1; push(@POS_ID,"$pos"); if($POS_CUMUL[$i] eq ""){ print "Inconsistent cumul. counts.\n";
exit; } if($POS_PHRED[$i] eq ""){ print "Inconsistent PHRED scores.\n"; exit; }
...
print OUT "set ylabel \"$numsq_ylabel\" $numsq_offset,0 $ylabel_font textcolor $c_dgreen\n";
print OUT "set y2label '$cumul_ylabel' $cumul_offset,0 $ylabel_font textcolor $c_red\n";
