...
##########################################################################################
#                                                                                        #
# Script  : analyze_10x_single_cell_v1_3_reads_centos7.pl
# Author  : Christophe Magnan                                                            #
# Modified by: Yuzo Kanomata
# Updated : 2018/06/22                                                                   #
#                                                                                        #
##########################################################################################
...
# Script Inputs
my $script_usage="./analyze_paired_reads.pl  fastqR1  fastqR2  fastqR3 prefix  [sample]";
if(($#ARGV!=2)&&($#ARGV!=3)){ print "\nUsage: $script_usage\n\n"; exit; }
my $fastqR1=$ARGV[0]; my $fastqR2=$ARGV[1]; my $fastqR3=$ARGV[2]; my $prefix=$ARGV[3]; my $library="$prefix";
my $multiplex=0; my $barcode=""; if($#ARGV==4){ $library=$ARGV[4]; } if($prefix=~"-"){
my @d=split("-",$prefix); foreach my $cand (@d){ my $len=length($cand);
if(($len>=4)&&($len<=10)&&($cand=~/^[ACGT]+$/)){ $multiplex=1;
...
if(! -f "$fastqR3"){ print "Input Fastq File 2 Not Found.\n"; exit; }
...
my $gnuplot_callsR2_out="$prefix-READ3-BaseComposition.png";
...
my $num_POS_R3=0; my @POS_ID_R3; my @POS_PHRED_R3; my %POS_CALLS_R3;
...
my $callsR3_min=1; $callsR3_max=0; my $callsR3_ytics=0.1;
...
if($l=<IN3>){ if($l!~/^@/){ print "Input 3 not in FastQ file format.\n"; exit; } }

else{ print "Inputs are not 3-reads.\n"; exit; } 

$l=<IN1>; 
chomp($l); 
my @nts1=split("",$l);
my $len1=$#nts1+1; 
if($num_POS_R1==0){ $num_POS_R1=$len1; } elsif($len1!=$num_POS_R1){
print "Sequences must have same length.\n"; exit; } 
for(my $i=0;$i<$len1;$i++){ my $nt=$nts1[$i];
if($VALID{"$nt"}!=1){ print "Invalid base found $nt.\n"; exit; } $POS_CALLS_R1{"$i-$nt"}++; }

$l=<IN2>; chomp($l); my @nts2=split("",$l); my $len2=$#nts2+1; if($num_POS_R2==0){
$num_POS_R2=$len2; } elsif($len2!=$num_POS_R2){ print "Sequences must have same length.\n"; exit; }
for(my $i=0;$i<$len2;$i++){ my $nt=$nts2[$i]; if($VALID{"$nt"}!=1){ print "Invalid base found $nt.\n";
exit; } $POS_CALLS_R2{"$i-$nt"}++; } 

$l=<IN1>; $l=<IN1>; chomp($l); my @phred1=split("",$l);
if(($#phred1+1)!=$len1){ print "Scores length mismatch 1.\n"; exit; } for(my $i=0;$i<$len1;$i++){
$POS_PHRED_R1[$i]+=(int(ord($phred1[$i]))-$offset); } 

$l=<IN2>; $l=<IN2>; chomp($l); my @phred2=split("",$l); 
if(($#phred2+1)!=$len2){ print "Scores length mismatch 2.\n"; exit; }
for(my $i=0;$i<$len2;$i++){ $POS_PHRED_R2[$i]+=(int(ord($phred2[$i]))-$offset); } 

$num_reads++; }

if(my $l=<IN2>){ print "Inputs are not 3-reads.\n"; exit; } close(IN1); close(IN2);
